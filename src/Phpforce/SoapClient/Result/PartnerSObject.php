...
class PartnerSObject extends SObject implements JsonSerializable{
    public $type;
    public $fields;
//  public $sobject;

    public function __construct($response=NULL, $client) {
            ...
            try {
                ...
                if ($response->any instanceof stdClass) {
                    if ($this->isSObject($response->any)) {
                        $anArray = array();
                        $sobject = new PartnerSObject($response->any);
                        $anArray[] = $sobject;
                        ...
                        foreach ($response->any as $key=>$item) {
                            if ( is_object( $item ) ) {
                                if ( $item instanceof SObject ) {
                                    $sobject = new PartnerSObject($item, $client);
                                    ...
                                    $children = [];
                                    $iterator = new RecordIterator( $client, $item );
                                    foreach( $iterator as $val ){
                                        $sobject = new PartnerSObject($val, $client);
                                        $sobject->Id = $val->Id;
                                        $children[] = $sobject;
                                    }
                                    $anArray[$key] = $children;
                                }
                            } else {
    ...
    //flatten data during serialisation
    public function jsonSerialize(){
        $ret = ['attributes'=>['type'=>$this->type, 'Id'=>$this->id],'Id'=>$this->Id ];
